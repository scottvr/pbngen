...
def generate(
    ...
    # Write raster output using shared primitives
...
def validate_output_dir(
    ...
    """
    Ensure output directory exists and protect against overwriting key files.

    Args:
        output_dir (Path): Directory where output will be stored.
        overwrite (bool): Whether to allow overwriting existing files.
        expect (list): List of expected output filenames.

    Returns:
        dict: Mapping of output labels to full paths (e.g. {"vector": Path(...)})
    """
